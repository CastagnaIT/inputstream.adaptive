...
bool CSubtitleSampleReader::IsReady()
{
  ...
  return !m_adByteStream || (m_adByteStream && m_adStream &&
                             !(m_adStream->getRepresentation()->flags_ &
                               adaptive::AdaptiveTree::Representation::WAITFORSEGMENT) == 0);
}
...
AP4_Result CSubtitleSampleReader::ReadSample()
{
  ...
  else if (m_adByteStream && m_adStream) // Read the sample data from a segment file stream (e.g. HLS)
  {
    ...
    else if ((m_adStream->getRepresentation()->flags_ &
              adaptive::AdaptiveTree::Representation::WAITFORSEGMENT) == 0)
    {
